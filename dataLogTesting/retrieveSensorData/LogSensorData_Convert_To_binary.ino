  ...
  if (Serial.available()) {
    ...
    if (filename.length() > 0) {
      ...
      if (myReader.openLogFile(filename.c_str())) {
        ...
        int entryCount = 0;
        // CSV header
      Serial.println("Timestamp_ms,imu_i,imu_j,imu_k,imu_real,Barometric_Pressure,Airspeed_Data");
        
        
        // Loop through the file, reading one entry at a time
        while (myReader.readNextEntry(logEntry)) {
          entryCount++;
           Serial.print(logEntry.timestamp_ms);
          Serial.print(',');
          Serial.print(logEntry.imu_data.i);
          Serial.print(',');
          Serial.print(logEntry.imu_data.j);
          Serial.print(',');
          Serial.print(logEntry.imu_data.k);
          Serial.print(',');
          Serial.print(logEntry.imu_data.real);
          Serial.print(',');
          Serial.print(logEntry.baro_data.pressure);
          Serial.print(',');
          Serial.println(logEntry.airspeed_data.air);
          //Serial.println("--------------------");
          //Serial.print("Entry #");
